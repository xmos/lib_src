...
The application uses a number of tasks to send and receive audio samples, convert from serial (streaming) samples to block buffers, timestamp and monitor the stream rates and of course sample rate convert asynchronously. In addition, there are a number of control tasks to complete the application including configuration of the codecs via I2C and GPIO, reading of the buttons and displaying a matrix of 4 x 4 multiplexed LEDs. 
...
The audio interfaces require I/O ``ports`` to communicate with the external components, as well as ``clock blocks`` to manage the rate of sampling and outputs. All ports and clock blocks are declared on the tile they reside since the chip used in the hardware has two tiles. 
...
The rate of the I2S interface is initialised to 44100 but can be changed at run-time by pressing button 1. Each press of the button cycles through 44.1KHz -> 48KHz -> 88.2KHz -> 96KHz and wraps back to the beginning.
